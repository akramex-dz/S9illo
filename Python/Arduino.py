...
class Arduino:
    ...
    def __init__(self, db, id, address, waterLevelSensor, pinWaterLevel=0, powerPinWaterLevel=0):
        ...
        self.stream_handler_add_plant_counter = 0
        #setattr(self.stream_handler_add_plant, 'counter', 0)
        self.stream_add_plant = self.db.child("raspberries").child("1234").child(
            "tabArduino").child(str(self.id_arduino)).child("plants").stream(self.stream_handler_add_plant)

    def stream_handler_add_plant(self, message):
        self.stream_handler_add_plant_counter += 1

        if(str(message["path"]) == "/" and self.stream_handler_add_plant_counter > 1):
            plant_ids = self.db.child("raspberries").child("1234").child(
                "tabArduino").child(str(self.id_arduino)).child("plants").shallow().get()
            ...
            plant = self.db.child("raspberries").child("1234").child("tabArduino").child(
                str(self.id_arduino)).child("plants").child(str(new_plant_id)).get()
            plant = plant.val()

            arduino = self.db.child("raspberries").child("1234").child(
                "tabArduino").child(str(self.id_arduino)).get()
            arduino = arduino.val()
    ...
    def recuperer_valeur_sol(self, pin, power_pin, plant):
        ...
        while not stop and cpt < 3:
            ...
            for i in range(0, 6):  # l'information contient 6 characters
                string += chr(rep[i])
            # est egale a 1 si la l'information a etait bien envoyer et 0 si l'information n'etait pas bien envoyer
            pass_bien = string[1]
